                      ...
                      char *tipo)
{
    ...
    *tipo = ' ';
    ...
    {
        //printf ("\nen while de resolver");
        if (sis.numinc > sis.numecu)
            *tipo = 'D';
        else
        {
            pos = buscarfilanocero (sis, i);
            //si no encuentro ninguna fila distinta de cero
            if (pos == -1)
                *tipo = 'D';
            else
            {
...
void comprobarsistema (tiposistema *sis, int f, char *tipo)
{
    ...
    //tengo que comprobar las filas desde f hasta la ultima
    i = f;
    while (i < sis->numecu && *tipo ==' ')
    {
        ...
        if (cont == 0)
        {
            //o tengo la igualdad 0=0
            //o tengo la igualdad 0=1
            if (sis->t[i][sis->numinc] != 0)
                *tipo = 'I';
            else
            {
                //tengo que quitar la fila i del sistema
                //intercambio la ultima fila con la fila i y
                //le resto 1 al numero de ecuaciones
...
void calcularsolucion (tiposistema sis, tiposolucion sol)
{
    ...
    {
        //solucion incognita i despejada en fila i
        //inicializo la suma al termino independiente
        suma = sis.t[i][sis.numinc];
        //paso los coeficientes de las demas incognitas al otro lado
        //del igual como negativos y los multiplico por la incognita
        for (j=i+1; j<sis.numinc; j++)
        {
            suma = suma - sis.t[i][j] * sol[j];
        }
        //divido entre el coeficiente de la variable i
