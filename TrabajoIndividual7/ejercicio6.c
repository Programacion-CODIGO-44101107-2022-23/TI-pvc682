...
double calcularvolumen (double (*f) (double x, double y), double xmin, double xmax, double ymin, double ymax, int m, int n)
{
    ...
    for (i=1; i<=m; i++)
    {
        //la esquina inferior izquierda seria   xi = xmin + (i-1) * lx;   yj = ymin + (j-1) * ly;
        //pero como piden el punto medio del area de cada cuadradito de longitud lx x ly
        //el punto medio es esa posicion x + lx/2    y + ly/2
        xi = xmin + (i-1) * lx + lx/2;
        for (j=1; j<=n; j++)
        {
            yj = ymin + (j-1) * ly + ly/2;
